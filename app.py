...
# --- AUTHENTICATION ---
from src.auth import sign_in_with_email_and_password, sign_up_with_email_and_password
    ...
    if auth_mode == "Login":
        if st.button("Login"):
            ...
            if "idToken" in res:
                ...
                st.error(f"Login failed: {res.get('error', {}).get('message', 'Unknown error')}")
    else:
        if st.button("Sign Up"):
            ...
            if "idToken" in res:
                ...
                st.error(f"Signup failed: {res.get('error', {}).get('message', 'Unknown error')}")
    st.stop()
...
with st.sidebar:
    if st.button("Logout"):
        ...
        st.session_state["messages"] = [AIMessage(content="Hello! Upload a dataset and ask me anything about it.")]
        st.rerun()
